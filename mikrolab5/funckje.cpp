...
int dodaj_kontener(unsigned long long int rozmiar, TElement * element, wchar_t * sciezka){
	...
	element = 0;
	if (!rozmiar) {
		// pomin pliki o zerowej wielkosci
		return 0;
	}
...
int generuj_raport(TOpcje *opcje){
	...
	while (glowa) {
		...
		while(elem) {
			pod_elem = elem->nast;
			while(pod_elem) {
				if (!MD5Cmp(elem->MD5, pod_elem->MD5)) {
					// sa co najmniej 2 identyczne pliki
					if(!elem->zaznaczony){
						// zapisz MD5 i pierwszy element
						MD5ToWStr(WMD5,elem->MD5);
						dlugosc = (unsigned int)wcslen(WMD5);
						WriteFile(hPlik,WMD5,dlugosc * 2,&written,NULL);
						WriteFile(hPlik,enter.enter,enter.dlugosc *2,&written,NULL);
						dlugosc = (unsigned int)wcslen(elem->sciezka);
						WriteFile(hPlik, elem->sciezka, dlugosc * 2,&written,NULL);
						WriteFile(hPlik,enter.enter,enter.dlugosc *2,&written,NULL);
						elem->zaznaczony = true;
					}
					if (!pod_elem->zaznaczony) {
						dlugosc = (unsigned int)wcslen(pod_elem->sciezka);
						WriteFile(hPlik, pod_elem->sciezka, dlugosc * 2,&written,NULL);
						WriteFile(hPlik,enter.enter,enter.dlugosc *2,&written,NULL);
						pod_elem->zaznaczony = true;
						straty += glowa->rozmiar;
					}
				}
				pod_elem = pod_elem->nast; // naste
			}
			elem = elem->nast; // element z ktorym porownujemy reszte listy elementow
		}
		glowa = glowa->nast; // przechodzenie po kontenerach
	}
	WriteFile(hPlik,(char*)&straty,8,&written,NULL);
	CloseHandle(hPlik);
	return 0;
}
...
// konwersja input (MD5) binarnego na znaki typu wide char
...
// konwersja input (MD5) binarnego na znaki typu char
